 ...
 * parentheses. Each `TypedExpr.raw(s)` otherwise allocates a fresh `Fragment` + `AppliedFragment` per call, and a
 * single `.compile` can walk through a dozen of them (`" WHERE "`, `" AND "`, `", "`, `" FROM "`, …).
 *
 * These are process-wide singletons. Safe to share across all threads — `AppliedFragment` is immutable and carries no
 * state beyond the (interned) SQL string and the argument list.
 *
 * Constructed via direct `Fragment` / `AppliedFragment` calls — deliberately does NOT route through `TypedExpr.raw`,
 * which now looks up these constants and would circularly initialise as `null`.
 */
object RawConstants {

  /**
   * Process-wide intern table: maps a SQL string to a single shared `AppliedFragment`. Populated by the `TypedExpr.raw`
   * macro for every compile-time-constant call site, so a `raw("(")` anywhere in the codebase resolves to the same
   * `AppliedFragment` instance regardless of which file it appears in.
   *
   * Bounded by source-code size — only compile-time-known strings flow into this map, so it cannot grow with runtime
   * input. Truly dynamic strings (table aliases, runtime operator symbols, `whereRaw` / `havingRaw` payloads) skip this
   * and go through `rawDynamic`, which always allocates fresh.
   */
  ...
  /**
   * Process-wide counter of fresh `AppliedFragment` allocations driven by runtime-built SQL strings — `s"…"`
   * interpolations against variables, `whereRaw` / `havingRaw` payloads. Read by benchmarks / diagnostics to track
   * progress toward the "everything except whereRaw/havingRaw is static" goal. The counter is best-effort — concurrent
   * compiles may race on increments, but the order-of-magnitude count over millions of compiles is what matters.
   */
   ...
   * (null) so production has zero overhead.
   */
  @volatile private[sharp] var rawDynamicByCallSite: java.util.concurrent.ConcurrentHashMap[String, Long] = null

  /**
   * Bounded array caches for `" LIMIT n"` and `" OFFSET n"` AppliedFragments. Pagination patterns reuse a small fixed
   * set of values (10, 20, 25, 50, 100, …); pre-allocating the first `LimitOffsetCacheSize` entries lazily covers the
   * realistic spread without unbounded memory growth. Larger values fall back to `rawDynamic` and allocate fresh per
   * compile.
   */
