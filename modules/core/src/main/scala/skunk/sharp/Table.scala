...
final case class Table[Cols <: Tuple, Name <: String & Singleton](
   ...
   * `users.insert(row)` where every column is supplied. Skips the per-compile `s"…"` interpolation + column-name
   * `mkString` that `InsertCommand.headerAf` would otherwise rebuild every time. Subset inserts (`.insert(partialRow)`
   * with `withDefault`-elided columns) bypass this cache and build dynamically.
   */
