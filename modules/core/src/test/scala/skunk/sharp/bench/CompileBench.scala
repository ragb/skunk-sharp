...
/**
 * Rough micro-benchmark / dynamic-AF accounting: runs `.compile` on representative SELECT, INSERT, UPDATE, DELETE, and
 * JOIN shapes and reports per-call-site dynamic-AppliedFragment counts plus rough timing. Not JMH — run manually with
 * `sbt "core/runMain skunk.sharp.bench.CompileBench"`. Good enough for before/after sanity checks while we push work
 * from runtime into the macro / interning path.
 *
 * The dynamic-AF count is the load-bearing metric: each `RawConstants.rawDynamic` invocation indicates a
 * compile-time-unknown SQL fragment that allocates a fresh `AppliedFragment`. Driving the per-compile count to zero
 * (outside `whereRaw` / `havingRaw` payloads) is the design goal of this PR.
 */
object CompileBench {
