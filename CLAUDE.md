...
- `table.renamed("other")` reuses a declaration (columns, constraints, schema) under another name — partitions, archive / per-tenant tables. Partitioned tables need nothing else: the parent is a plain `Table` (`information_schema` reports `BASE TABLE`).
...
- `Merge.scala` — extension on `Table`. `stock.merge(source)` → `MergeBuilder` (only `.on`) → `MergeCommand[…, Ready]` with `.whenMatched` / `.whenNotMatched` / `.whenNotMatchedBySource` (each optionally conditioned) → action (`.update` / `.delete` / `.doNothing`, `.insert` for not-matched). `Ready` flips to `true` after the first branch; `.compile` / `.returning*` check it via `inline if constValue[Ready]`. `Closed <: Tuple` lists branch kinds closed by an unconditional branch (`Merge.CloseIf`); `Merge.requireOpen` makes a later branch of that kind a compile error. INSERT value types are checked through `ExprFits` (`@implicitNotFound` names the column). Branch lambdas get only what Postgres allows: matched = joined view, not-matched = source `ColumnsView`, by-source = target `ColumnsView`; SET lambdas use `SetView` (by-source) / `MergeSetView` (matched: target `SetView` + source as read-only `SourceColumn`s whose `:=` is a compile error). All branches fold into one `Fragment[CArgs]` as they're added. `Pg.mergeAction` for RETURNING.

**Joins carry builder state.** `.xJoin(…)` on a `SelectBuilder` hands the builder to `IncompleteJoin` (`carried`), and `.on` rebuilds with its WHERE / GROUP BY / HAVING / ORDER BY / LIMIT / OFFSET / DISTINCT (ON) / locking and their Args types. Clauses written on a single source render unqualified names — correct, or a loud Postgres "ambiguous column" error, never a silent rebind. LATERAL lambdas always get an alias-qualified outer view (`lateralView`), since inner names shadow outer ones. UPDATE … FROM SET lambdas see FROM sources as read-only `SourceView`s.
...
**Named parameters** ([`Named.scala`](modules/core/src/main/scala/skunk/sharp/Named.scala)): `Param.named["x", T]` is a `NamedParam` whose Args slot is `Named["x", T]` — a tiny final wrapper class (not an opaque alias: match types must prove it disjoint from `Void` / tuples / plain types, and casts to it must be sound). Inside the DSL it's an ordinary positional slot; the execute surface in `Compiled.scala` (`run`/`unique`/`option`/`stream`/`cursor`/`prepared`/`bind`/`*K`) takes `NamedArgs.RunArgs[Args]` — a named tuple of distinct labels when every slot is named, else the positional Args with named slots taking a bare `T` — and `NamedArgs.toSlots` remaps/wraps (no-op when no slot is named). Same label with two types → `SameType` compile error. `Pg.unnestRows[Row]("label")` is the named batch form. Guideline: named for complex / repeated / same-typed-swappable params, positional for simple statements.
...
**Infix arithmetic** ([`ops/Arith.scala`](modules/core/src/main/scala/skunk/sharp/ops/Arith.scala)): `+ - * / %`, unary `-`, rendered parenthesised. Result type from per-operator typeclasses (`Plus` / `Minus` / `Times` / `Div` / `Mod`, over `Stripped` operand types; numeric promotion in `ArithOut`'s companion) lifted to `Option` by `ArithResult` when either side is nullable; temporal instances too. Extension types add instances via `Plus.of` etc. in their own companion (pgvector does). The integration `ArithSuite` verifies the promotion table against Postgres (skunk's column-alignment check fails on a wrong result type). Export note: `*` must be backquoted in the `dsl` export clause.

**Primitive literals direct (SIP-71 `into`)**: `TypedExpr` is declared `into trait` and ships singleton-typed `given Conversion`s in its companion, so `column === "x"`, `column >= 18`, `column.between(1, 10)`, `column.like("%foo%")`, `:=` in UPDATE SET, etc. all accept primitive literals directly with no `lit(…)` wrap and no `language.implicitConversions` import. SQL is byte-identical to the `lit`-wrapped form. Limits: only primitives (`Boolean`/`Byte`/`Short`/`Int`/`Long`/`Float`/`Double`/`String`), only singleton-typed sources (literals + stable `val`s, not method calls / vars / widened ascriptions), only at direct `TypedExpr` slots (collection elements like `.in(NonEmptyList.of(…))` still need `lit(…)` because the element type is inferred as the raw `T`). Non-primitive types (UUID, dates, tag types) keep needing `lit(v)`. For runtime values: `=== Param[T]` (deferred), `=== Param.bind(v)` (Void-bake). Requires `-preview` (set project-wide; stabilises in 3.9).
